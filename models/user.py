...
class User(Base):
    ...
    points_totale = Column(Numeric(2), default=0)
    ...
    @classmethod
    def create_user(cls, session, username:str, email:str, password:str, phone_number:str,profile_image:str,id_:str,lalitude:float,longitude:float,role:str, encryption_key:str)->bool:
    ...
    @classmethod
    def get_all_users_ordered_by_total_points(cls, session) -> list:
        """
        Get all users with role 'USER' ordered by their total points.

        Parameters:
            session: SQLAlchemy session object.

        Returns:
            list: A list of user objects ordered by their total points.
        """
        return session.query(cls).filter(cls.role.lower() == 'user').order_by(desc(cls.points_totale)).all()
